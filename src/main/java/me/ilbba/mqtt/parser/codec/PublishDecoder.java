...
public class PublishDecoder extends DemuxDecoder {
    ...
    public void decode(AttributeMap ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        //Topic name
        String topic = CodecUtils.decodeString(in);
        if (topic == null) {
            in.resetReaderIndex();
            return;
        }
        ...
        //read the payload
        int payloadSize = remainingLength - (stopPos - startPos - 2) + (CodecUtils.numBytesToEncode(remainingLength) - 1);
