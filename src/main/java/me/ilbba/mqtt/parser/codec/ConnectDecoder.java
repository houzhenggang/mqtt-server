...
public class ConnectDecoder extends DemuxDecoder {
    ...
    @Override
    public void decode(AttributeMap ctx, ByteBuf in, List<Object> out) throws UnsupportedEncodingException {
        in.resetReaderIndex();
        ...
        switch (protocolNameLen) {
            case 6:
                ...
                versionAttr.set((int) CodecUtils.VERSION_3_1);
                break;
            case 4:
                ...
                message.setProtocolName(protoName);
                versionAttr.set((int) CodecUtils.VERSION_3_1_1);
                break;
            default:
                //protocol broken
        ...
        if (message.getProtocolVersion() == CodecUtils.VERSION_3_1_1) {
        ...
        //int keepAlive = CodecUtils.readWord(in);
        int keepAlive = in.readUnsignedShort();
        message.setKeepAlive(keepAlive);

        if ((remainingLength == 12 && message.getProtocolVersion() == CodecUtils.VERSION_3_1) ||
                (remainingLength == 10 && message.getProtocolVersion() == CodecUtils.VERSION_3_1_1)) {
            out.add(message);
            return;
        }

        //Decode the ClientID
        String clientID = CodecUtils.decodeString(in);
        if (clientID == null) {
            in.resetReaderIndex();
            return;
        }
        ...
        //Decode willTopic
        if (willFlag) {
            String willTopic = CodecUtils.decodeString(in);
            if (willTopic == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode willMessage
        if (willFlag) {
            byte[] willMessage = CodecUtils.readFixedLengthContent(in);
            if (willMessage == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode username
        if (userFlag) {
            String userName = CodecUtils.decodeString(in);
            if (userName == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode password
        if (passwordFlag) {
            byte[] password = CodecUtils.readFixedLengthContent(in);
            if (password == null) {
                in.resetReaderIndex();
                return;
            }
